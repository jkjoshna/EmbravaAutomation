...
test('Verify the give feedback dropdown options ', async ({ testLogin, testBaseurl }) => {
 test.setTimeout(60000); // ← extend test limit
  ...
  const commonFunctions = new CommonFunctions(page,testLogin, testBaseurl);
    ...
    await page.getByRole('combobox').selectOption('I have an idea');



    await expect(page.locator('[data-automation="submitBTN"]').first()).toHaveText('Send feedback');
    //await expect(page.locator('h2')).toHaveText('Share your feedback');
