...
test('Verify the add and remove functionality for desk devices', async ({ testLogin, testBaseurl }) => {
  ...
  // await page.waitForTimeout(4000);
  // Wait for the added device row to be visible in the table.

  const rowLocator = page.getByRole('row').filter({ hasText: '5666435436' }).first();
  //await expect(rowLocator).toBeVisible({ timeout: 15000 });

  // Check the checkbox within that specific row
  await page.locator('input[type="checkbox"]').nth(1).check();
  //await rowLocator.getByRole('checkbox').check();
